...
define('http', ['jquery'], function ($, londing) {
    ...
    $(function () {
        ...
        function git(data) {
            var goods1 = data.map(function (item, idx) {
                return '<li>' + '<a href=\'./goods.html' + '?id=' + item.id + '\'><img src = \'' + item.imgurl + '\'/></a>' + '<p>' + item.title + '</p>' + '<p>\uFFE5' + item.price + '元</p>' + '</li>';
            });
            return goods1;
        }
        if (window.localStorage.getItem('name')) {
            ...
            http.post('./api/car.php', { uname: name }).then(function (res) {
                ...
                var goods = data.map(function (item, id) {
                    return '<tr class="gs">' + '<td width="25%"><a href="goods.php ?id=' + item.id + '  "><img width="60px" src="' + item.imgurl + '"></a></td>' + '<td class="overtxt" width="50%"><a href="goods.php ?id=' + item.id + '  ">' + item.title + '</a><br>礼物款式:狐狸蓝 直接购买</td>' + '<td class="tr" width="25%"> x' + item.qty + '<br>\uFFE5' + item.price + '<br><a class="remove_item" data-rec-id="1339972" href="javascript:void(0)">删除</a></td>' + '</tr>';
                });
                $('.showlist').html(goods);
